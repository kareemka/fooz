...
const ProductsListingContent = () => {
    ...
    const { data, loading: isLoading, fetchMore } = useQuery(GET_PRODUCTS, {
        variables: {
                ...
                <Container>
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6 w-full md:w-auto order-1 md:order-2">
                            <div className="flex items-center gap-2 bg-white/5 px-4 py-3 md:py-2 rounded-xl border border-white/10 w-full sm:w-auto transition-colors focus-within:bg-white/10 focus-within:border-primary/50">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        const timer = setTimeout(() => handleSearch(e), 500);
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
                        ...
                        <aside className="lg:col-span-1 space-y-8">
                            ...
                            {/* Categories Filter */}
                            <div className="glass-aaa border border-white/5 p-8 rounded-[2rem] text-right">
                                <h3 className="text-white font-black mb-6 flex items-center justify-end gap-3 translate-x-1">
                                    <span>التصنيفات</span>
                                ...
                                <div className="space-y-4">
                                    <button
                                        ...
                                        className={`flex items-center justify-between group cursor-pointer w-full text-right ${!activeCategorySlug ? 'text-primary' : 'text-gray-400'}`}
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className={`w-5 h-5 rounded-lg border flex items-center justify-center transition-colors ${!activeCategorySlug ? 'border-primary bg-primary/20' : 'border-white/10 bg-white/5'}`}>
                                                <div className={`w-2 h-2 bg-primary rounded-sm ${!activeCategorySlug ? 'opacity-100' : 'opacity-0'}`} />
                                            </div>
                                            <span className={`text-sm font-bold transition-colors ${!activeCategorySlug ? 'text-white' : 'group-hover:text-white'}`}>الكل</span>
                                        </div>
                                    </button>
                                    {categories.map((cat) => (
                                        <button
                                            ...
                                            className={`flex items-center justify-between group cursor-pointer w-full text-right ${activeCategorySlug === cat.slug ? 'text-primary' : 'text-gray-400'}`}
                                        >
                                            <div className="flex items-center gap-4">
                                                <div className={`w-5 h-5 rounded-lg border flex items-center justify-center transition-colors ${activeCategorySlug === cat.slug ? 'border-primary bg-primary/20' : 'border-white/10 bg-white/5'}`}>
                                                    <div className={`w-2 h-2 bg-primary rounded-sm ${activeCategorySlug === cat.slug ? 'opacity-100' : 'opacity-0'}`} />
                                                </div>
                                                <span className={`text-sm font-bold transition-colors ${activeCategorySlug === cat.slug ? 'text-white' : 'group-hover:text-white'}`}>{cat.name}</span>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                            ...
                            {/* AR Feature Promo */}
                            <div className="relative group cursor-pointer overflow-hidden rounded-[2rem] aspect-[4/3]">
                                <img src="https://images.unsplash.com/photo-1593305841991-05c297ba4575?q=80&w=400&auto=format&fit=crop" alt="AR Promo" className="w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700" />
